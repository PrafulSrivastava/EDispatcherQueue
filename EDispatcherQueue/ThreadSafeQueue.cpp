...
ThreadSafeQueue::ThreadSafeQueue() {
	...
	for (int i = 0; i < QUEUE_SIZE; i++) {
		workerthreads[i] = std::thread(&ThreadSafeQueue::p_ReadDataAsync, this);
	}
}
...
void ThreadSafeQueue::v_WriteData(std::shared_ptr<Event> p_Evt) {
	logger->print("Sync Write..");
	p_Evt->v_EventHandler();
}

void ThreadSafeQueue::v_WriteDataAsync(std::shared_ptr<Event> p_Evt) {
...
std::shared_ptr<Event> ThreadSafeQueue::p_ReadData() {
	logger->print("Why? What to do");
}
...
void ThreadSafeQueue::p_ReadDataAsync() {
